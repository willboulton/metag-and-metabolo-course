...
We'll analyse the data using R, but the codes to reproduce figures from the paper are also available in [this github repository](https://github.com/apduncan/dime_analysis). 
...
- The point of this course is to be 'hands-on' in its approach. However, if you just want to engage with the results, there is a shortcut. Clone [this repository](https://github.com/apduncan/dime_analysis) and follow the instructions provided in the README (you won't need to re-create the R environment if you followed along with the steps, we'll have done this already) - the whole pipeline will be generated using the `targets` library, and using the function `tar_make()`. 
